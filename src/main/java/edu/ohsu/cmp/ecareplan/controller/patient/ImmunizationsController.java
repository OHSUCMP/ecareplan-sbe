...
import edu.ohsu.cmp.ecareplan.model.dataset.ImmunizationModel;
...
public class ImmunizationsController extends BasePatientController {
    ...
    public String view(HttpSession session, Model model) throws Exception {
        ...
        if (sessionService.exists(sessionId)) {
            ...
            model.addAttribute("dataSets", DataSet.IMMUNIZATIONS);
    ...
    public ResponseEntity<List<IProgress>> getProgress(HttpSession session) {
        return userWorkspaceService.exists(session.getId()) ?
                ResponseEntity.ok(userWorkspaceService.get(session.getId()).getCurrentProgress(DataSet.IMMUNIZATIONS)) :
                ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
    }

    @PostMapping("models")
    public ResponseEntity<List<ImmunizationModel>> getModels(HttpSession session) {
        return userWorkspaceService.exists(session.getId()) ?
                ResponseEntity.ok(userWorkspaceService.get(session.getId()).getAllDataSetModels(DataSet.IMMUNIZATIONS)) :
                ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
    }
