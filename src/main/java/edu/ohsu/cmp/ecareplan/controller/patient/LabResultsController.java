...
public class LabResultsController extends BasePatientController {
    ...
    @PostMapping("models")
    public ResponseEntity<List<Consolidated<LabResultModel>>> getModels(HttpSession session) {
        return userWorkspaceService.exists(session.getId()) ?
                ResponseEntity.ok(consolidate(userWorkspaceService.get(session.getId()).getAllDataSetModels(DataSet.LAB_RESULTS))) :
                ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
    }
