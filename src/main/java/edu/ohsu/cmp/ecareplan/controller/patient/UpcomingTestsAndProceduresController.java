...
@Controller
@RequestMapping("/patient/upcoming-tests-and-procedures")
public class UpcomingTestsAndProceduresController extends BasePatientController {
    ...
    public String view(HttpSession session, Model model) throws Exception {
        ...
        if (sessionService.exists(sessionId)) {
            ...
            model.addAttribute("dataSets", DataSet.SERVICE_REQUESTS);

            auditService.doAudit(sessionId, AuditSeverity.INFO, "visited /patient/upcoming-tests-and-procedures");
    ...
    public ResponseEntity<List<IProgress>> getProgress(HttpSession session) {
        return userWorkspaceService.exists(session.getId()) ?
                ResponseEntity.ok(userWorkspaceService.get(session.getId()).getCurrentProgress(DataSet.SERVICE_REQUESTS)) :
                ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
    }

    @PostMapping("models")
    public ResponseEntity<List<ServiceRequestModel>> getModels(HttpSession session) {
        return userWorkspaceService.exists(session.getId()) ?
